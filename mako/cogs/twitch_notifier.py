...
class TwitchNotification(commands.Cog):
        ...
        for joined_guild in self.bot.guilds:
            ...
            if not watchlist:
                ...
                async for user in users:
                    ...
                    profile_image_list = (
                        await database_manager.get_saved_profile_images(user.login)
                    )
    ...
    async def set_notification_channel(self, ctx, notification_channel_id: str) -> None:
        await ctx.respond(
            await database_manager.set_notification_channel(
                ctx.guild.id, int(notification_channel_id)
            )
        )
    ...
    async def get_notification_channel(self, ctx) -> None:
        ...
        await ctx.respond(
            f"The following Channel is set as Notification Channel on Guild {ctx.guild}: {watchlist}"
        )
